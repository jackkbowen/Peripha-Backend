...
// Used for logout and other functions that don't require a protected route for a specific user
function verifyUserAccessAnyUser(req, res, done) {
  ...
  verifyJWT(token, req, res, done).then(async (username) => {
    ...
    console.log(err);
    res.status(500).send({ message: 'Some error occurred in verifyUserAccessAnyUser. Please try again.' });
  });
}
...
function verifyUserAccess(req, res, done) {
  ...
  verifyJWT(authToken, req, res, done).then(async (username) => {
    ...
    if ((!req.body.username) && (!req.params.username)) {
      ...
      const userData = await Reviews.findById(reviewId).select('username');
    ...
    const comparisonUsername = req.params.username ?? req.body.username;
